...
//All combinations of 3 out of 5 community cards - 10
	...
	for (int i1 = 0; i1 < 5; ++i1) {
		for (int i2 = i1 + 1; i2 < 5; ++i2) {
			for (int i3 = i2 + 1; i3 < 5; ++i3) {
				//cout << i1 << " " << i2 << " " << i3 << " \n";
				entry[0] = i1; entry[1] = i2; entry[2] = i3;
...
//All combinations of 2 out 4 hole cards - 6
...
public:
	...
	int flushCombinations(std::vector<Card> mrg) {
		...
		bool seqCards5 = (isDistinct(mrg, 0, 4) && (mrg[4].r_vv - mrg[0].r_vv) == 4) ? true : false;
		//Do we have four sequential cards 
		int seqCards4 = (isDistinct(mrg, 0, 3) && (mrg[3].r_vv - mrg[0].r_vv) == 3) ? true : false;
		...
		if (seqCards5 || seqCards4) {
			...
			//if we have 4 cards in sequential order check if it is special case 2-3-4-5-A
				...
				if (seqCards4 && mrg[0].r_vv == 0 && mrg[4].r_vv == 12) {
	...
	int otherCombinations(std::vector<Card> mrg, int *sameRankCount) {
		...
		else if (seqCards4 && mrg[0].r_vv == 0 && mrg[4].r_vv == 12)
		{
			cout << " Straight with Ace lowest\n";
			score = STRAIGHT;
	...
	//hole.size + community.size = HAND_SIZE 
	int scoreHand(vector<Card> hole, vector<Card> community) {
		...
		if (hole.size() + community.size() > HAND_SIZE)
			throw std::exception("Hand size cannot exceed {1} cards", HAND_SIZE);
		...
		{
			sameRankCount[mrg[i].r_vv]++;
			sameSuitCount[mrg[i].s_vv]++;
...
public:
	...
	void addCard(Card card)
	{
		if (hole.size() < HOLE_CARDS)
			...
			throw std::exception("Number dealt cards cannot exceed {1}", HOLE_CARDS);
	}
